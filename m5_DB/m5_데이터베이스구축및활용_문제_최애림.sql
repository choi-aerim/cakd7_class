...
--A.
--1) 뷰 생성
create view vw_employees_temp as
select employee_id, last_name, salary, hire_date, trunc(months_between(sysdate, hire_date)/12,0) "근속년수"
from employees;
...
select sum(puramt) "총 구매액",
       sum(select puramt from purprd where substr(purdate, 1, 4) = 2014) "2014 구매액",
       sum(select puramt from purprd where substr(purdate, 1, 4) = 2015) "2015 구매액"
from purprd;
